...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	ms := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(ms, 2.0)/height*cmInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
